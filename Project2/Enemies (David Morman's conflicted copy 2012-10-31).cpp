...
int Enemies::findSmallestDist(Enemies elementals[]) {
	...
	if(canMove(UP, elementals) && abs(currentMove - UP) != 2){
		...
		checkDistance[0] = getDistance(x+16, y, x+16, y-16);
	}
	else{
		dist[0] = 0;
		checkDistance[0] = 0;
	...
	if(canMove(LEFT, elementals) && abs(currentMove - LEFT) != 2){
		...
		checkDistance[1] = getDistance(x, y+16, x-16, y+16);
	}
	else{
		dist[1] = 0;
		checkDistance[1] = 0;
	...
	if(canMove(DOWN, elementals) && abs(currentMove - DOWN) != 2){
		...
		checkDistance[2] = getDistance(x+16, y+32, x+16, y+48);
	}
	else{
		dist[2] = 0;
		checkDistance[2] = 0;
	...
	if(canMove(RIGHT, elementals) && abs(currentMove - RIGHT) != 2){
		...
		checkDistance[3] = getDistance(x+32, y+16, x+48, y+16);
	}else{
		dist[3] = 0;
		checkDistance[3] = 0;
	}
